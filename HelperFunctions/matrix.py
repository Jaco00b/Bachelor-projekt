...
import numbers
from HelperFunctions.kernels import MaternKernel, MaternKernel_vec, MaternKernel_mtx, GaussianKernel, GaussianKernel_vec, GaussianKernel_mtx, LaplaceKernel, LaplaceKernel_vec, LaplaceKernel_mtx
...
class AbstractPSDMatrix(ABC):
    ...
    def __matmul__(self, other):
        if isinstance(other, np.ndarray):
            if other.ndim == 2:  # Matrix multiplication
                n, m = other.shape
                result = np.zeros((self.shape[0], m))
                for i in range(m):
                    result[:, i] = self[:, :] @ other[:, i]
                return result
            elif other.ndim == 1:  # Vector multiplication
                return self[:, :] @ other
            else:
                raise ValueError(f"Unsupported array dimension: {other.ndim}")
        else:
            return self[:, :] @ other
        
    def __add__(self, other):
        if isinstance(other, (int, float)):
            # If other is a scalar, treat it as a scaled identity matrix
            return PSDMatrix(self[:,:] + other * np.eye(self.shape[0]))
        else:
            # For other types of addition
            return PSDMatrix(self[:,:] + other)

    def __rmul__(self, other):
        if isinstance(other, (int, float)):
            # Handle scalar multiplication
            return PSDMatrix(other * self[:,:])
        else:
            raise TypeError(f"Unsupported operand type for *: '{type(other)}' and 'KernelMatrix'")
